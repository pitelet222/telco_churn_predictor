...
from app.llm_client import aget_retention_advice
...
async def advice(payload: AdviceRequest) -> AdviceResponse:
    """Predict churn and generate LLM-powered retention recommendations."""
    # 1. Run the prediction (CPU-bound, stays sync)
    try:
    ...
    # 2. Call the LLM for retention advice (I/O-bound, now async)
    try:
        llm_advice = await aget_retention_advice(
